...
#define CONFIG_ELOGD_LOCK_PATH    "/run/lock/elogd"
#define CONFIG_ELOGD_KMSG_FETCH   (16U)
#define CONFIG_ELOGD_SVC_FETCH    (16U)
...
#define elogd_assert_conf() \
	elog_assert(elogd_conf.kmsg_fetch > 0); \
	elog_assert(elogd_conf.mqueue_fetch > 0); \
	elog_assert(elogd_conf.svc_fetch > 0); \
	elog_assert(!(elogd_conf.svc_mode & ~DEFFILEMODE)); \
	elog_assert(!elogd_conf.svc_group || elogd_conf.svc_group[0]); \
	elog_assert(upath_validate_path_name(elogd_conf.dir_path) > 0); \
	elog_assert(upath_validate_path_name(elogd_conf.stat_path) > 0); \
	elog_assert(elogd_conf.file_len); \
	elog_assert((size_t)upath_validate_file_name(elogd_conf.file_base) == \
	            elogd_conf.file_len); \
	elog_assert(!(elogd_conf.file_mode & ~DEFFILEMODE)); \
	elog_assert(!elogd_conf.file_group || elogd_conf.file_group[0]); \
	elog_assert(elogd_conf.max_size >= ELOGD_FILE_SIZE_MIN); \
	elog_assert(elogd_conf.max_size <= ELOGD_FILE_SIZE_MAX); \
	elog_assert(elogd_conf.max_rot); \
	elog_assert(elogd_conf.max_rot <= 100)
...
{
	elog_assert(head);
	elog_assert(!(severity & ~LOG_PRIMASK));
	elog_assert(!(facility & ~LOG_FACMASK));
...
#define elog_assert_line_head(_line, _iovec) \
	elog_assert((_iovec)[ELOGD_LINE_HEAD_IOVEC].iov_len <= \
	            sizeof((_line)->head)); \
	elog_assert((char *)(_iovec)[ELOGD_LINE_HEAD_IOVEC].iov_base >= \
	            (_line)->head); \
	elog_assert((char *)(_iovec)[ELOGD_LINE_HEAD_IOVEC].iov_base < \
	            &(_line)->head[sizeof((_line)->head)])

#define elog_assert_line_msg(_line, _iovec) \
	elog_assert((_iovec)[ELOGD_LINE_MSG_IOVEC].iov_len); \
	elog_assert((_iovec)[ELOGD_LINE_MSG_IOVEC].iov_len < \
	            sizeof((_line)->data)); \
	elog_assert((char *)(_iovec)[ELOGD_LINE_MSG_IOVEC].iov_base > \
	            (_line)->data); \
	elog_assert((char *)(_iovec)[ELOGD_LINE_MSG_IOVEC].iov_base < \
	            &(_line)->data[sizeof((_line)->data)])

#define elog_assert_queued_line(_line) \
	elog_assert(!((_line)->severity & ~LOG_PRIMASK)); \
	elog_assert(!((_line)->facility & ~LOG_FACMASK)); \
...
elogd_fixup_partial_line(struct elogd_line * __restrict line, size_t written)
{
	...
	elog_assert(written);
	elog_assert(written < (line->vector[ELOGD_LINE_HEAD_IOVEC].iov_len +
...
{
	elog_assert(queue);
	elog_assert(queue->nr);
	elog_assert(queue->lines);
	elog_assert(queue->busy_cnt <= queue->nr);
	elog_assert(!!queue->busy_cnt ^ dlist_empty(&queue->busy));
...
elogd_dqueue_line(struct elogd_queue * __restrict queue)
{
	...
	elog_assert(queue->nr);
	elog_assert(queue->lines);
	elog_assert(queue->busy_cnt);
	elog_assert(queue->busy_cnt <= queue->nr);
	elog_assert(!dlist_empty(&queue->busy));
                        ...
                        unsigned int                   count)
{
	...
	elog_assert(count);
	elog_assert(count <= queue->busy_cnt);
                         ...
                         size_t * __restrict            size)
{
	...
	elog_assert(queue->busy_cnt <= queue->nr);
	elog_assert(!dlist_empty(&queue->busy));
	elog_assert(dlist_empty(lines));
	elog_assert(vectors);
	elog_assert(max_cnt);
	elog_assert(max_cnt <= queue->busy_cnt);
	elog_assert((2 * max_cnt) <= IOV_MAX);
	elog_assert(size);
	elog_assert(*size);
	elog_assert(*size <= SSIZE_MAX);
                         ...
                         unsigned int                    count)
{
	...
	elog_assert(!!queue->busy_cnt ^ dlist_empty(&queue->busy));
	elog_assert(count);
	elog_assert((queue->busy_cnt + count) <= queue->nr);
	elog_assert(!dlist_empty(lines));
                          ...
                          size_t                          written)
{
	...
	elog_assert(vectors);
	elog_assert(count);
	elog_assert((2 * count) <= IOV_MAX);
	elog_assert((queue->busy_cnt + count) <= queue->nr);
	elog_assert(written < SSIZE_MAX);
	...
	elog_assert(cnt < count);
	elog_assert(!cnt || ((node != lines) && (node != first)));
...
elogd_open_store_file(struct elogd_store * __restrict store)
{
	elogd_assert_conf();
	elog_assert(store);
	elog_assert(store->dir >= 0);
	elog_assert(upath_validate_path_name(store->base) >
	            (ssize_t)(elogd_conf.file_len + 1));
	elog_assert(store->base[elogd_conf.file_len] == '.');
	elog_assert(store->base[(2 * elogd_conf.file_len) + 1 + 2 + 1] == '.');
...
elogd_rotate_store(struct elogd_store * __restrict store)
{
	elogd_assert_conf();
	elog_assert(store);
	elog_assert(store->fd >= 0);
	elog_assert(store->dir >= 0);
	elog_assert(upath_validate_path_name(store->base) >
	            (ssize_t)(elogd_conf.file_len + 1));
	elog_assert(store->base[elogd_conf.file_len] == '.');
	elog_assert(store->base[(2 * elogd_conf.file_len) + 1 + 2 + 1] == '.');
	elog_assert(elogd_conf.max_rot > 1);
                        ...
                        size_t                          size)
{
	...
	elog_assert(queue);
	elog_assert(max_cnt);
	elog_assert(elogd_queue_busy_count(queue) == max_cnt);
	elog_assert(size);
	elog_assert(size <= SSIZE_MAX);
	...
	elog_assert(ret != -EINTR);
	elog_assert(ret != -EINVAL);
...
elogd_open_store(struct elogd_store * __restrict store)
{
	...
	if (fstatvfs(store->dir, &stat) < 0) {
		...
		elog_assert(err != -EBADF);
		elog_assert(err != -EFAULT);
		elog_assert(err != -EINTR);
...
{
	elog_assert(line);
	elog_assert(line->vector[ELOGD_LINE_MSG_IOVEC].iov_len);
...
{
	elog_assert(kmsg);
	elog_assert(kmsg->dev_fd >= 0);
	elog_assert(kmsg->seqno);
	elog_assert(kmsg->queue);
	elog_assert(kmsg->stat_fd >= 0);
                    ...
                    const struct upoll *  poll __unused)
{
	...
	elog_assert(!(state & EPOLLRDHUP));
	elog_assert(!(state & EPOLLPRI));
	elog_assert(state & (EPOLLIN | EPOLLERR));
	elog_assert(poll);
...
elogd_open_kmsg_stat(struct elogd_kmsg * __restrict kmsg)
{
	...
	if (seqno == MAP_FAILED) {
		...
		elog_assert(err != -EBADF);
		elog_assert(err != -EINVAL);
		elog_assert(err != -EOVERFLOW);
                ...
                const struct upoll * __restrict poll)
{
	...
	fd = ufd_open("/dev/kmsg",
	              O_RDONLY | O_CLOEXEC | O_NOCTTY | O_NOFOLLOW | /*O_NOATIME |*/
	              O_NONBLOCK);
	if (fd < 0)
		return fd;
               ...
               struct elogd_line * __restrict      line)
{
	...
	elog_assert(!(msg.msg_flags & MSG_EOR));
	elog_assert(!(msg.msg_flags & MSG_OOB));
	elog_assert(!(msg.msg_flags & MSG_ERRQUEUE));
               ...
               const struct upoll * __restrict poll)
{
	...
	err = unsk_svc_bind(&svc->unsk, CONFIG_ELOGD_SOCK_PATH);
	umask(msk);
	if (err)
		goto close;
...
{
	elog_assert(mqueue);
	elog_assert(mqueue->fd >= 0);
	elog_assert(mqueue->lines);
	elog_assert(line);
